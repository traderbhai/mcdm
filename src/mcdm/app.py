...
if not edited_criteria_scores.empty:
    ...
    normalization_factor["NormalizedScore"] = normalization_factor["NormalizedScore"] ** 0.5

    normalized_weighted = melted_df.merge(normalization_factor, on="Criterion", how="left")
    normalized_weighted["NormalizedScore"] = normalized_weighted["Score"] / normalized_weighted["NormalizedScore"]
    normalized_weighted["NormalizedWeightedScore"] = (
        normalized_weighted["NormalizedScore"] * normalized_weighted["Weight"]
    )
    normalized_weighted["Is Negative"] = normalized_weighted["Is Negative"].fillna(False)

    positive_criteria = normalized_weighted[~normalized_weighted["Is Negative"]]
    best_positive = (
    ...
    negative_criteria = normalized_weighted[normalized_weighted["Is Negative"]]
    best_negative = (
    ...
    normalized_weighted["EuclidianDistanceBestPartial"] = (
        normalized_weighted["NormalizedWeightedScore"] - normalized_weighted["IdealBest"]
    ) ** 2
    normalized_weighted["EuclidianDistanceWorstPartial"] = (
        normalized_weighted["NormalizedWeightedScore"] - normalized_weighted["IdealWorst"]
    ) ** 2

    euclidian = (
        normalized_weighted.groupby("Option")[["EuclidianDistanceBestPartial", "EuclidianDistanceWorstPartial"]]
        .sum()
        .reset_index()
    )
    euclidian["EuclidianDistanceBest"] = euclidian["EuclidianDistanceBestPartial"] ** 0.5
    euclidian["EuclidianDistanceWorst"] = euclidian["EuclidianDistanceWorstPartial"] ** 0.5
